...
namespace XUnitTests
{
    ...
    public class TestMySql
    {
        ...
        public void InsertRandomData_Count_ModelString()
        {
            ...
            for (int i = 0; i < 99; i++)
            {
                db.Insert(new ModelString { String_test = "azerty" });
        ...
        public void InsertRandomDataList_Count_ModelString()
        {
            ...
            for (int i = 0; i < 99; i++)
            {
                list.Add(new ModelString { String_test = "azerty" });
        ...
        public void SelectFirst_Contains_test_ModelString()
        {
            ...
            var tests2 = db.Select<ModelString>(x => x.String_test.Contains("_test"));
            tests2 = db.Select<ModelString>(x => x.String_test.Contains("test"));
            tests2 = db.Select<ModelString>(x => x.Id == 1);
            Assert.NotNull(tests2);

            ModelString ms = db.SelectFirst<ModelString>(x => x.String_test.Contains("_test"));
            Assert.NotNull(ms);
            Assert.Equal(1, ms.Id);

            ms = db.SelectFirst<ModelString>(x => x.String_test.Contains("test"));
            Assert.NotNull(ms);
            Assert.Equal(1, ms.Id);
        }
